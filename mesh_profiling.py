...
class ProfilingResultDatabase:
    ...
    def update(self, new_database):
        cluster_key = 'default'
        print("new_database.data.item(): ", new_database.data.item())
        #print("new_database.item(): ", new_database.item())
        _new_database = dict(new_database.data.item())
        for mesh_shape, mesh_result in _new_database.items():
            print(f"mesh_shape, mesh_result: {mesh_shape}- {mesh_result}")
    ...
    def load(self, filename):
        import io
        new_data = np.load(io.BytesIO(open(filename, 'rb').read()), allow_pickle=True)
        #with open(filename, "rb") as f:
        #    new_data = np.load(io.BytesIO(open(auto_stage_option.cached_profile_result, 'rb').read()), allow_pickle=True)
            #new_data = np.load(f, allow_pickle=True)
...
def estimate_hlo_module_cost(hlo_module,
    ...
    model."""
    with XlaPassContext({
            "gpu_cost_model::profiling_results": None, #profiling_results,
        ...
        return xe.estimate_hlo_module_cost(hlo_module) # EstimateHloModuleCost

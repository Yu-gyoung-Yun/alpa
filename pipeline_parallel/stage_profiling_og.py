...
class HloCostModelProfileWorker:
    ...
    def __init__(self, num_devices, num_micro_batches):
    ...
    def profile(self, stage_id, compiled_module_output, stage_plan,
        ...
        hlo_module = compiled.hlo_modules()[0] # sharding + SPMD optimization
        ...
        cost = estimate_hlo_module_cost(hlo_module, self.prof_result, # runtime 예측
...
class HloCostModelProfileWorkerPool(BaseWorkerPoolWrapper):
    ...
    def __init__(self, num_cpus, placement_group, prof_result, mesh_num_devices,
        ...
        self.actors = [
            ...
            ).remote(mesh_num_devices, num_micro_batches) #(prof_result, mesh_num_devices, num_micro_batches)
...
def generate_module_profile_result(raw_result: Tuple,
    ...
    compute_costs = np.mean(compute_costs) # SAMPLE VARIANCE
...
def profile_all(stages, compiled_outputs: Sequence[CompileOutput], meshes,
    ...
    for _ in pbar:
        try:
            ((i, module_id),
             *module_raw_result) = profile_workers.get_next_unordered() # stage_id, cost, peak_memory, available_memory
            print(f"i-module_id-{i}=={module_id}={module_raw_result}")
        except TimeoutError:
    ...
    for mesh_id, submesh in reversed(list(enumerate(submesh_choices))):
        ...
        if global_config.profile_with_whole_ray_cluster:
            ...
            sliced_virtual_meshes = virtual_mesh.slice_profiling_submeshes(
                num_hosts, num_devices_per_host)
        for config_idx, auto in enumerate(autosharding_configs[mesh_id]): # ray를 사용하면서 이렇게 수정해야해서 global_compile time이 생김.

...
def training_dp_impl(num_layers, num_devices, num_microbatches, submesh_choices,
    ...
    for s in range(1, num_layers + 1):  # pylint: disable=too-many-nested-blocks
        for i in range(num_layers - 1, -1, -1):
            for j in range(1, num_devices + 1):
                for k in range(num_layers, i, -1): # i (layer_num부터 하나씩 줄어들어, 0까지 감) ------ k --------num_layers (마지막 layer까지)
                    for m, submesh in enumerate(submesh_choices): # ((1,1), (1,2), (1,4), (1,8))
                            ...
                            for n_config in range(num_autosharding_configs):
                                if s - 1 <= max_n_succ_stages[i, k - 1, m,
                                    ...
                                    stage_cost = compute_cost[i, k - 1, m,
                                                              n_config]
                                    new_cost = f[s - 1, k, j - # k(i~num_layers를 아우르는) 의 compute cost + Layer i~Layer k-1까지의 stage cost
                                                 n_submesh_devices] + stage_cost # communication cost더하지 않고 그냥 pipeline stage끼리의 runtime을 더해
                                    if (stage_cost <= max_stage_cost and
                                            new_cost < f[s, i, j]): # min runtime
                                        f[s, i, j] = new_cost # pipeline-stage라는 뜻이 결국에는 sequential하게 runtime을 더하는 것(dependency); s를 한 stage에 들어가는 layer 조합으로 보면 되나?
    ...
    for s in range(1, num_layers + 1):
        if f[s, 0, num_devices] < best_total_cost: # min_runtime과 비교
            best_s = s
...
def get_submesh_choices(
    ...
    # smaller submeshes:
    i = 1
    while i <= num_devices_per_host: # (1,1), (1,2), (1,4)
    ...
    if space == "all":
        ...
        i = 2
        while i <= num_hosts:
            submesh_choices.append((i, num_devices_per_host)) # (2,4), (4,4)
            i *= 2
...
def get_one_submesh_autosharding_config_choices(
    ...
    if space in ["all", "single_node_model_parallel"]:
        if space == "all":
            max_mp_dimension = num_devices # len(self.host_ids) * self.num_devices_per_host # 8
        else:  # space == "single_node_model_parallel"
            max_mp_dimension = virtual_submesh.num_devices_per_host # 4 # here

        for mp_size in range(1, max_mp_dimension + 1): # 1,2,3,4
            if num_devices % mp_size == 0:
                dp_size = num_devices // mp_size # 8, 4, 2
        ...
        results.append((virtual_submesh.get_logical_mesh((1, num_devices)), { # (1,8)
            "force_batch_dim_to_mesh_dim": 0
        }))
    else:
...
def cluster_layers_and_slice_mesh(
    ...
    if isinstance(stage_option, AutoStageOption):
        ...
        autosharding_configs = get_all_submesh_autosharding_config_choices(
            ...
            stage_option.submesh_logical_shape_space, batch_size) # "single node model parallel"
        ...
        if inference_mode:
            ...
            _, solution = training_dp(num_layers, virtual_mesh.num_devices,
                                      ...
                                      num_autosharding_configs, compute_cost,
                                      max_n_succ_stages)
        search_end = time() - search_start
        print("search time: ", search_end)
